...
def book_information(
    trs,
):  # Création d'un dictionnaire pour récupérer les product information
    sub_book = {}
    for tr in trs:
        th = tr.find("th").text
        td = tr.find("td").text
...
def scrape_book(url):
    book = (
        {}
    )  # dictionnaire, les {} peuvent être remplacé par dict() pour plus de clarté
    ...
    book["price_including_tax"] = information[
        "Price (incl. tax)"
    ]  # sub_book['Price (incl. tax)'] 			#price_including_tax
    book["price_excluding_tax"] = information[
        "Price (excl. tax)"
    ]  # price_excluding_tax
